...
async def run_test():
    ...
    try:
        ...
        # -> Input admin email into the email field (index 25) and password into the password field (index 26), then click the 'Giriş Yap' button (index 29).
        ...
        # -> Click the 'Yeni Alan Ekle' (Yeni Alan Ekle) button to open the add-venue form (index 521).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/main/div/main/div/div/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Fill the Title (TR) field with 'Test Mekan' (index 715) and submit the form by clicking 'Alanı Kaydet' (index 728). Then verify the created venue appears in the venues list.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div/div/main/div/main/div/div/div[2]/form/div[10]/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('Test Mekan')
        ...
        elem = frame.locator('xpath=/html/body/div[1]/div/main/div/aside/nav/button[1]')
        assert await elem.is_visible()
        elem = frame.locator('xpath=/html/body/div[1]/div/main/div/main/div/div/div[1]/button')
        assert await elem.is_visible()
        text = (await elem.inner_text()).strip()
        if "Yeni Mekan Ekle" not in text:
            raise AssertionError(f"Expected 'Yeni Mekan Ekle' to be present, but found '{text}'. Feature may not exist. Marking task done.")
