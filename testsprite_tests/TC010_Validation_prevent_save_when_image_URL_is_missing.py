...
async def run_test():
    ...
    try:
        ...
        # -> Fill the email and password fields and click the 'Giriş Yap' button to log in (use indexes 21, 22, then click index 25).
        ...
        # -> Open the Add Artifact form by clicking 'Yeni Eser Ekle' (quick action) to start creating a new artifact record.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/main/div/main/div/div/div[3]/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Click the 'Yeni Eser Ekle' button (index 518) to open the Add Artifact form/modal so the category, title and save controls become available.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/main/div/main/div/div/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Select 'Fotoğraflar & Objeler' from the Kategori dropdown, fill Başlık (TR) with 'Eksik Görsel TR', leave the image input empty, click 'Eseri Kaydet', then check that validation text containing 'Görsel' and 'zorunlu' is visible.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div/div/main/div/main/div/div/div[2]/form/div[4]/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('Eksik Görsel TR')
        ...
        frame = context.pages[-1]
        await expect(frame.locator('text=Görsel').first).to_be_visible(timeout=3000)
