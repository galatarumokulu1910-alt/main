...
async def run_test():
    ...
    try:
        ...
        # -> Type the email and password into their inputs and click the 'Giriş Yap' button to log in.
        ...
        # -> Click the 'Yeni Kategori' button to open the new main category form (index 524).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/main/div/main/div/div/div[3]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Click the 'Kaydet' button (index 649) to attempt saving the main category with required fields empty and observe whether a visible validation message (e.g., 'Zorunlu') appears.
        frame = context.pages[-1]
        # Click element
        ...
        frame = context.pages[-1]
        frame = context.pages[-1]
        elem = frame.locator('xpath=/html/body/div[1]/div/main/div/main/div/div/div[3]/form/div[3]/button[1]').nth(0)
        assert await elem.is_visible(), 'Expected the form cancel (İptal) button to be visible, but it is not — form may not be open.'
        # The expected "Kaydet" button or the validation message "Zorunlu" is not present in the available elements; report issue and stop.
        assert False, 'Kaydet button or validation text "Zorunlu" not found on the page; feature may be missing.'
