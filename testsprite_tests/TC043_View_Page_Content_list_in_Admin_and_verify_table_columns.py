...
async def run_test():
    ...
    try:
        ...
        # -> Type the admin email into the E-POSTA field (index 179), then type the password into the ŞİFRE field (index 183), then click the Giriş Yap button (index 186).
        ...
        # -> Click the 'Sayfa İçerikleri' (Page Content) tab (index 280) to open the content list and then verify the presence of 'Sayfa' and 'Bölüm' columns or labels.
        frame = context.pages[-1]
        # Click element
        ...
        elem = frame.locator('xpath=/html/body/div/div/main/div/aside/nav/button[8]').nth(0)
        assert await elem.is_visible(), "Expected text 'Sayfa' to be visible but it is not"
        raise AssertionError("Element containing text 'Bölüm' was not found in the available elements. Feature may be missing.")
