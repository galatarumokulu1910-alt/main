...
async def run_test():
    ...
    try:
        ...
        # -> Type the admin email into the E-posta field and the password into the Şifre field, then click Giriş Yap.
        ...
        frame = context.pages[-1]
        await expect(frame.locator('text=Admin panel').first).to_be_visible(timeout=3000)
        await expect(frame.locator('text=Yıl').first).to_be_visible(timeout=3000)
        await expect(frame.locator('text=Başlık (TR)').first).to_be_visible(timeout=3000)
