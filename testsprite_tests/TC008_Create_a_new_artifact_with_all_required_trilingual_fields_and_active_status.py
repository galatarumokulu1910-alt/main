...
async def run_test():
    ...
    try:
        ...
        # -> Type the admin email into the email field (index 24) and fill the password, then click the 'Giriş Yap' button to log in.
        ...
        # -> Open the Artifacts module by clicking the 'Arşiv Eserleri' sidebar item so the artifacts listing page is shown.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/main/div/aside/nav/button[3]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Click the 'Arşiv Eserleri' (Artifacts) sidebar item to open the artifacts listing page.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/main/div/aside/nav/button[2]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Click the 'Yeni Eser Ekle' (Add New Artifact) button to open the artifact creation form (click element index 692).
        frame = context.pages[-1]
        # Click element
        ...
        await expect(frame.locator('text=Test Eser TR').first).to_be_visible(timeout=3000)
        await expect(frame.locator('text=Aktif').first).to_be_visible(timeout=3000)
