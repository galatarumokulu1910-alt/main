...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Tarihçe' tab in the sidebar to open the Tarihçe module (element 276).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/main/div/aside/nav/button[5]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Click the 'Yeni Olay Ekle' (+ Yeni Olay Ekle) button to open the new history entry form (element 518).
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/main/div/main/div/div/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Fill the Yıl, Görünüm Sırası, Başlık TR and Açıklama TR fields and click 'Olayı Kaydet' to save the new history entry.
        frame = context.pages[-1]
        # Input text
        ...
        await page.wait_for_timeout(3000); await elem.fill('Sıralama testi')
        
        # -> Fill the Açıklama (TR) textarea with 'Otomatik test açıklaması' (index 702), then click 'Olayı Kaydet' (index 706) to save the new history entry.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div/div/main/div/main/div/div/div[2]/form/div[3]/div[2]/textarea').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('Otomatik test açıklaması')
        ...
        frame = context.pages[-1]
        frame = context.pages[-1]
        elem = frame.locator('xpath=/html/body/div/div/main/div/main/div/div/div[2]/table/tbody/tr[3]/td[5]/div/button[1]').nth(0)
        assert await elem.is_visible()
        elem = frame.locator('xpath=/html/body/div/div/main/div/main/div/div/div[2]/table/tbody/tr[3]/td[5]/div/button[2]').nth(0)
        assert await elem.is_visible()
