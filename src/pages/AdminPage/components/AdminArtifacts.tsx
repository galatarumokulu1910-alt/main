...
export default function AdminArtifacts() {
    ...
    const fetchData = async () => {
        ...
        try {
            const [catsRes, subcatsRes, artsRes] = await Promise.all([
                ...
                `).neq('archive_type', 'istanbul_rum').order('created_at', { ascending: false })
            ]);
    ...
    const saveItem = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (isEditing && currentId) {
                res = await supabase.from('artifacts').update(payload).eq('id', currentId);
            } else {
                res = await supabase.from('artifacts').insert([payload]);
            }
            if (res.error) throw res.error;
            await fetchData();
            resetForm();
        } catch (error: any) {
            console.error("Eser kaydedilirken hata oluştu:", error);
            alert("Eser kaydedilirken bir hata oluştu: " + (error.message || error));
            setLoading(false);
        }
    };
    ...
    if (loading && artifacts.length === 0) return <div>Eserler yükleniyor...</div>;

    return (
        <div className="admin-module">
            <div className="admin-module-header">
                <h2>Arşiv Eserleri</h2>
                {!isEditing && (
            ...
            {isEditing ? (
                <div className="admin-form-card">
                    <div className="admin-form-card-header">
                        ...
                        <h3>{currentId ? 'Eseri Düzenle' : 'Yeni Eser'}</h3>
                    </div>
                ...
                <div className="admin-table-container">
                    <table className="admin-table">
                        ...
                        <tbody>
                            {artifacts.length === 0 ? (
                                <tr className="admin-empty-row"><td colSpan={5}>Henüz hiç eser eklenmemiş.</td></tr>
                            ) : (
                                artifacts.map(art => (
                                    <tr key={art.id}>
                                        <td>
